...
submitButton.addEventListener("click", () => {
    ...
    if (productName.value < 6) {
        canBeSubmited = false;
        error[0].textContent = "Your product's name must have more than 5 characters";
        error[0].style.display = "block";
    } else error[0].style.display = "none";

    if (droppingZone.children[1].style.display != 'none') {
        canBeSubmited = false;
        error[1].textContent = "Your product must have an image";
        error[1].style.display = "block";
    } else error[1].style.display = "none";
    
    if (Number(price.textContent.slice(0,-1).replaceAll(",", "")) < 50) {
        canBeSubmited = false;
        error[2].textContent = "Your product's price must be more than 50 pesos";
    ...
    if (productDescription.value.length < 50) {
        canBeSubmited = false;
        error[3].textContent = "Your product's description must have more than 50 characters";
        error[3].style.display = "block";
    } else error[3].style.display = "none";

    if (product.filters.length < 1) {
        canBeSubmited = false;
        error[4].textContent = "Your product must have at least one tag";
        error[4].style.display = "block";
    } else error[4].style.display = "none";

    if (productType.selectedIndex == 0 || productColor.selectedIndex == 0) {
        canBeSubmited = false;
        error[5].textContent = "Your must select one option";
    ...
    if (canBeSubmited) {
        ...
        .then(res => {
            if (res[0] == "FE") {
                ...
                window.location.replace('/' + languaje + "/product#" + res[0]);
            }
        });
    }
});
...
if (userString === null) {
    ...
    window.location.replace('/' + languaje + '/'); 
} else {
