...
const observer2 = new IntersectionObserver(entries => {    
    if (entries[0].isIntersecting) {
        let i = 0;
        const func = () => {
            entries[0].target.children[i].style.transform = "translateX(0)";
            //entries[0].target.children[i].addEventListener("mouseover", (e) => {
            //    e.target.style.transform += 'scale(1.1)';
            //});

            //entries[0].target.children[i].addEventListener("mouseout", (e) => {
            //    e.target.style.transform -= 'scale(1.1)';
            //});
            i++;
            if (i > 3) {
