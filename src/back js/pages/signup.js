    ...
    return await new Promise(async (res) => {
        if (req.reason == "Existence") {
            ...
            */
            ...
            database.query(`select * from waitlist where email = '${req.email}'`, (e, resp) => {
                if (e) {
                    res("FE"); 
                    throw e;
                }
                ...
                if (req.value == resp[0].verifyCode) {
                    ...
                    database.query(`insert into users(email, username, passwrd) values("${resp[0].email}","${resp[0].username}","${resp[0].passwrd}")`, er => {
                        if (er) {
                            res("FE");
                            throw er;
                        };
