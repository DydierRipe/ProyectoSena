...
const main = async (req) => {
    return await new Promise(async (res) => {
        if (req.reason == "Existence") {
            ...
            let exist = await utilities.returnExistence(req, "waitingUsers.JSON", "email", "email"); // see if the user is in the waiting room
            ...
            */
            if (exist[0] !== "N") {
                console.log(exist[1].code, "si");
                if (exist[1].code === req.value) {
                    let response = await utilities.deleteFromFile(req, "waitingUsers.JSON"); // delete user of the waiting room
                    console.log("chi");
                    if (response === "CE") {
                        let sortedData = await utilities.returnOrgData(req, "users.JSON", "email"); // insert user and sort the users list
                        fs.writeFile("users.JSON", JSON.stringify(sortedData), async err => {
                            if (err) {
                                console.log(err); // error
                                res("FE");
                            } else {
                                let username = {"body":{"UN":req.username,"email":req.email}}
                                let SD = await utilities.returnOrgData(username, "usernames.JSON", "UN");
                                fs.writeFile("usernames.JSON", JSON.stringify(SD), async err => {
                                    if (err) {
                                        console.log(err); // error
                                        let rsp = await utilities.deleteFromFile(req, "waitingUsers.JSON"); // delete user of the waiting room
                                        if (rsp !== "CE") {
