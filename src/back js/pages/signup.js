            ...
            */
            if (exist[0] !== "N") {
                ...
                if (exist[1].code === req.value) {
                    let response = await utilities.deleteFromFile(req, "src/files/waitingUsers.JSON"); // delete user of the waiting room
                    console.log("chi");
                    if (response === "CE") {
                        let sortedData = await utilities.returnOrgData(req, "src/files/users.JSON", "email"); // insert user and sort the users list
                        fs.writeFile("src/files/users.JSON", JSON.stringify(sortedData), async err => {
                            if (err) {
                                console.log(err); // error
                                res("FE");
                            } else {
                                let username = {"body":{"UN":req.body.username,"email":req.body.email}}
                                let SD = await utilities.returnOrgData(username, "src/files/usernames.JSON", "UN");
                                fs.writeFile("src/files/usernames.JSON", JSON.stringify(SD), async err => {
                                    if (err) {
                                        console.log(err); // error
                                        let rsp = await utilities.deleteFromFile(req, "src/files/waitingUsers.JSON"); // delete user of the waiting room
                                        if (rsp !== "CE") {
