...
const returnExistence = (req, fName, value, EName) => {
    return new Promise(async (res) => {
        ...
        const exist = algorithm.searching(data, req[value], EName); // verify if the request exist in a file

        if (!exist) {
            res(["N", "noRes"]);
        } else {
            if (exist[0].password == req.password) { // password verification
                res(["NC", exist[0]]);
            } else {
...
const deleteFromFile = (req, fileName) => {
    return new Promise(async (res, rej) => {
        ...
        const exist = algorithm.searching(fileContent, req.email, "email"); // look if the email exist in the got data
